...
async function exportRepo(repoIdOrName?: string) {
  ...
  for (const repo of repos) {
    ...
    const commits = repo.commits as ExportCommit[];
    const decisions = repo.decisions as ExportDecision[];
    const stack = stringArray(repo.stack);

    const md = `# ${repo.name}\n\n- Path: \`${repo.path}\`\n- Main branch: ${repo.mainBranch ?? "unknown"}\n- Stack: ${stack.join(", ") || "not set"}\n- Commits captured: ${commits.length}\n\n## Recent commits\n\n${commits.map((commit: ExportCommit) => `### ${commit.shortHash} — ${commit.message}\n\n- Date: ${commit.committedAt.toISOString()}\n- Tags: ${stringArray(commit.tags).join(", ") || "none"}\n\n${commit.aiSummary ?? "No summary."}`).join("\n\n")}\n\n## Decisions\n\n${decisions.map((decision: ExportDecision) => `- **${decision.title}** — ${decision.description}`).join("\n") || "No decisions recorded."}\n`;

    await fs.writeFile(path.join(dir, "memory.md"), md, "utf8");
    indexLines.push(`- [${repo.name}](./${slug}/memory.md) — ${commits.length} recent commits exported`);
  }
